...
SUBDOMAIN_CONF="$HOME/Projects/projects/nginx/subdomains/gitea.$DOMAIN.conf"
...
write "$SUBDOMAIN_CONF" "
  ...
  # HTTPS vhost for gitea.$DOMAIN
  server {
    listen 443 ssl http2;
    listen [::]:443 ssl http2;
    server_name gitea.$DOMAIN;

    # Using the SAN self-signed cert from your nginx setup (includes gitea.$DOMAIN)
...
info "  • SSH    : ssh://git@gitea.$DOMAIN:222"
