...
SUBDOMAIN_CONF="$HOME/Projects/projects/nginx/subdomains/jenkins.$DOMAIN.conf"
...
write "$SUBDOMAIN_CONF" "
  ...
  server {
    ...
    proxy_request_buffering off;   # don't buffer request bodies (e.g., uploads)
    proxy_buffering off;           # stream responses (useful for console logs)
    proxy_read_timeout 1d;         # Jenkins builds stream for a long time
...
# --- UFW rules (safe) ---------------------------------------------------------
log "Configuring UFW (allow 50000)"
sudo ufw allow 50000/tcp  || true
